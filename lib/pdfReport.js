...
export async function generatePdf(
  ...
  try {
    logoDataUrl = await loadImageAsDataUrl("/gmi-logo-white.png");
  } catch {
    logoDataUrl = "";
  }
  ...
  if (deviceMachines.length > 0) {
    ...
    for (const machine of deviceMachines) {
      ...
      if (coreRows.length > 0) {
        ...
        doc.setFont("helvetica", "bold");
        doc.setFontSize(9);
        doc.text("Core Device Information", 72, currentY);
        currentY += 12;
      ...
      if (rawFields.length > 0) {
        ...
        doc.setFont("helvetica", "bold");
        doc.setFontSize(9);
        doc.text("Additional Data", 72, currentY);
        currentY += 12;
